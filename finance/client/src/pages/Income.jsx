            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id}>
                  ...
                  <td>
                    <button type="button" className="btn-ghost px-2 py-1 text-red-600" onClick={() => api.income.remove(r.id).then(load)}>
              ...
              {!rows.length && (
                <tr>
                  <td colSpan={4} className="muted py-8 text-center">No income in this period.</td>
                </tr>
              )}
            </tbody>
          </table>
